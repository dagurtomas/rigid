...
theorem exists_surjective_presentation_of_isAffinoidAlgebra (hA : IsAffinoidAlgebra K A) :
    ∃ (n : ℕ) (π : TateAlgebra K (Fin n) →ₐ[K] A), Function.Surjective π := by
  exact ⟨hA.presentation.n, hA.presentation.toAlgHom,
    hA.presentation.toAlgHom_surjective⟩
...
theorem exists_quotient_presentation_of_isAffinoidAlgebra (hA : IsAffinoidAlgebra K A) :
    ∃ (n : ℕ) (I : Ideal (TateAlgebra K (Fin n))),
      Nonempty ((TateAlgebra K (Fin n) ⧸ I) ≃ₐ[K] A) := by
  exact ⟨hA.presentation.n, hA.presentation.ideal, ⟨hA.presentation.equiv⟩⟩
